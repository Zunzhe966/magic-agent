# 尊者网络管理（Network Manager）

> macOS 桌面软件，定位是**本机网络的管理员**：**一键归序** + **按软件精确分流** + **云服务器 SSH 控制台** + **AI 客户端（MCP）控制入口**。
> （产品更名「尊者网络管理」在重构 P2 阶段落地，路线见 [docs/重构计划.md](./docs/重构计划.md)；设计立场见 [docs/设计.md](./docs/设计.md)。）
...
**本软件的做法**：开启即接管——审计本机网络混乱源（竞品进程、端口残留、系统代理逐服务状态），把出口收敛到两条物理隔离的确定路径，每个联网软件独立指定走代理/直连；关闭或还原时按账本回到接管前。（审计/账本/回滚闭环已于 v0.3.0 交付：只读体检 + 接管账本 + 对账不达标自动回滚 + 一键还原 + 漂移巡检；产品更名「网络管理」在 P2，进度如实标注于 docs/重构计划.md。）
...
### 3. 启动即清理混乱源（v0.3.0 起为完整接管闭环）

App 启动与每次开启代理时，自动检测并清理第三方代理软件进程（FlClash/Clash Verge/sing-box 等 24+12 个名单，只匹配可执行文件路径防误杀），让系统回到干净状态再建立自己的秩序。v0.3.0 已升级为「审计 → 记账 → 接管 → 对账不达标自动回滚 → 一键还原」的完整闭环：接管前可选逐项确认（设置页开关），接管中漂移巡检提示，还原按账本回到接管前原值。
...
- 「一键归序」闭环（体检审计 → 接管记账 → 对账不达标自动回滚 → 一键还原 → 漂移巡检）已在 v0.3.0（P1-1~P1-4）交付；异常退出残留由启动自检兜底恢复直连。五场景人工验收（P1-5）为发布前最后一步，清单见 docs/重构计划.md。
