...
- **接管账本红线（P1-2 落地后新增）**：`ledger.json` 的 schema（camelCase 键集）
  ...
  **启动清理 ≠ 接管**：App 启动 800ms 自动清理不记账（否则永远有"未结账本"假象），
  ...
  stop/exit 只结账**不回滚**——按账本回写系统代理是 restore_network（P1-4）的专属
  入口，结账路径（stop_proxy / RunEvent::Exit / settle）绝不偷加还原。
  损坏账本必须先保全 .corrupt 证据再从零继续，绝不静默丢证据。
- **还原红线（P1-3/P1-4 落地后新增）**：
  - 回放必须按账本 before【最初原值】逐服务恢复，恢复成"达标"不是还原。
  - 无账可回绝不瞎写：restore_network 无未结账本时只能退回"关系统代理回直连"，
    并如实说明"若接管前并非直连请手动恢复"，禁止假装还原成功。
  - 快照不完整/errors 非空的通道宁可不还原也要点名，禁止凭 unknown 瞎写网络配置。
  - 账本 process 项（被杀第三方进程）不可逆，还原结果必须如实列出，禁止空喊也禁止隐瞒。
  - reapply_takeover 双检前提：内核在跑 + 有未结账本，两者缺一律拒写（对死端口设代理=制造断网）。
- **漂移巡检红线（P1-4）**：巡检**只读只报绝不自动改写**——用户可能正手动调试网络，
  抢写比漂移更恶劣；处置权在用户（[重新归位]/[接受]）。内核未跑不巡检（崩溃处置权
  归 30s 看门狗，两线程不得抢方向）；无未结账本不巡检且必须解除遗留黄条。
  drift 是持续状态非一次性事件：get_status 读出不清空，显式处置（归位达标/接受/还原/再接管）才清。
  MCP 侧无常驻线程，status 惰性抽验必须自带守卫（有账且内核在跑），禁止无守卫放大 networksetup 开销。
